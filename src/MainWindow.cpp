...
LRESULT CALLBACK MainWindow::HistoryListWindowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (message == WM_MOUSEMOVE) {
        owner->m_keyboardHandler.OnHistoryMouseMove(window, POINT{GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)},
            owner->m_items, owner->m_historyList, owner->m_pinsList, owner->m_popupVisible,
            owner->m_settings.show_search);
    }
    ...
    if (message == WM_LBUTTONUP) {
        const int index = owner->m_keyboardHandler.HistoryItemAtPoint(
            ...
            owner->m_historyList, owner->m_pinsList, owner->m_items);
...
void MainWindow::ClearHistory(bool all) {
    ...
    try {
        if (all) m_database.DeleteAll();
        else m_database.DeleteUnpinned();
...
bool MainWindow::HasHistoryItems() const {
...
std::wstring MainWindow::NextPinKey() const {
...
void MainWindow::ApplySettings() {
    ...
    if (!SameHotKey(previous.open_hotkey, m_settings.open_hotkey) ||
        ...
        m_keyboardHandler.UnregisterGlobalHotKey(kHotkeyId);
        m_keyboardHandler.RegisterGlobalHotKey(kHotkeyId);
    }
    UpdateTrayIcon();
...
void MainWindow::OnPreviewCallback(void* context, sqlite3_int64 itemId, bool keyboard) {
    auto* window = static_cast<MainWindow*>(context);
    window->SchedulePreviewForItem(itemId, keyboard ? PreviewSource::Keyboard : PreviewSource::Mouse);
}
...
LRESULT MainWindow::OnCommand(UINT, WPARAM wParam, LPARAM lParam, BOOL& handled) {
    ...
    if ((command == kHistoryListControlId || command == IDC_HISTORY_PINS) && notification == LBN_SELCHANGE) {
        ...
        if (!m_loadingList && !m_mouseSelectionUpdate) {
            ...
            if (selected != LB_ERR) {
                const LRESULT itemData = SendMessageW(list, LB_GETITEMDATA, selected, 0);
                if (itemData != LB_ERR && static_cast<size_t>(itemData) < m_items.size()) {
                    ...
                    if (!selected_by_mouse) {
                        ...
                        if (m_previewWindow.IsVisible()) {
                            ...
                            SchedulePreviewForItem(m_keyboardHandler.GetActiveItemId(), PreviewSource::Keyboard);
                        }
                    }
                }
            }
...
LRESULT MainWindow::OnClipboardUpdate(UINT, WPARAM, LPARAM, BOOL&) {
    if (m_clipboardMonitor.OnClipboardUpdate() && m_popupVisible) {
        RefreshHistory(ReadWindowText(m_search));
    }
    return 0;
}
...
LRESULT MainWindow::OnSettingsChanged(UINT, WPARAM, LPARAM, BOOL& handled) {
    handled = TRUE;
    ApplySettings();
    return 0;
}
